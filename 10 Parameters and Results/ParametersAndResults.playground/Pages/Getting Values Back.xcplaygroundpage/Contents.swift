...
func countVideoMessage(name: String) -> String {
    let desiredVideoDuration = 40 // желаемая продолжительность видео
    let holidayVideoCount = 100 //

    let videoMessage = spaceAvailableMessage(eachVideoDuration: desiredVideoDuration, numberOfVideos: holidayVideoCount)//видео сообщение
    let namedVideoMessage = "Hey \(name)! \(videoMessage)"// количество видеосообщений
    return namedVideoMessage
}
print(countVideoMessage(name: "Lola"))
